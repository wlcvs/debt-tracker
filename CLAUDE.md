...
**`paid` on Debt** toggles whether the debt counts toward balance. **`src/lib/balance.ts`'s `balanceTotals` is the single definition of "devido"/"pago"** — never write a new balance sum at a call site, and don't reintroduce a `.paid` filter of your own. The rule it encodes:

- **valor pago** = sum of every `Payment`
- **valor devido** = sum of the `Debt` rows with `paid === false`

No subtraction between the two and no zero clamp. `Debt.paid` and `Payment` used to be two independent "this is settled" mechanisms applied on top of each other (`totalOwed = Math.max(0, unpaidDebts - payments)`), so marking installments paid *and* registering the matching payment deducted the same money twice, with the clamp rendering the negative result as `R$ 0,00` while unpaid debts were still listed on screen. Marking a debt paid is now the only thing that clears it from the owed total — which is exactly what the retroactive back-filling workflow relies on. `getDashboardOverview` follows the same model in SQL (`groupBy` with `paid: false`, and deliberately **no** per-person payment aggregate to subtract), so the dashboard's "Total a receber" and the person page can't diverge.

`balanceTotals` takes an optional `"YYYY-MM"` month key, which is how the person and public pages scope the summary to the month picked in the carousel (see "Month carousel" under "UI patterns").
...
- `splitInstallmentAmounts(total, count)` — divides the total into cents, putting any leftover cent on the ***first*** installments (matching how Brazilian card issuers split a purchase: 685,91 in 10x → 68,60 + 9× 68,59) so the sum always matches exactly what was typed.
...
`createDebt` (`src/lib/actions/debt.ts`) branches on whether `installments` is **present at all** — not on `installments > 1` — to create the whole group via `prisma.debt.createMany`. That's why the field is `.optional()` with no `.default(1)`: a form that never opened the Parcelar panel omits it and gets one plain ungrouped `Debt`, while an explicit `installments=1` is a deliberate request for a real 1/1 group (`(1/1)` title suffix, `installmentGroupId`, `installmentIndex`/`Total` of 1). Adding a `.default(1)` back would silently collapse those two cases. Each individual installment's `paid` flag can be set at creation time via `paidInstallments` (a JSON array of 1-based indexes), letting you record some/all installments as already paid without a separate step. A single (non-parceled) debt can also be created already `paid: true` the same way.
...
- **pdfjs-dist server-side `DOMMatrix` crash**: the legacy build's `DOMMatrix`/`Path2D` polyfill comes from `@napi-rs/canvas`, whose actual implementation lives in a platform-specific package (e.g. `@napi-rs/canvas-linux-x64-gnu`). When that binary isn't loadable, pdfjs logs a `Path2D` polyfill warning and then dies at module scope with `ReferenceError: DOMMatrix is not defined`. **The same symptom has two unrelated causes** — check which environment is failing before "fixing" it:
  - **Locally**: a known npm bug ([npm/cli#4828](https://github.com/npm/cli/issues/4828)) sometimes skips installing the optional platform package even though it's in `package-lock.json`, and the app doesn't notice until a PDF is actually parsed. Dev server log shows `Cannot load "@napi-rs/canvas" package: Cannot find native binding`, and it surfaces client-side only as a generic "Failed to fetch". Fix: `rm -rf node_modules && npm install` and confirm `node_modules/@napi-rs/canvas-linux-x64-gnu` (or your platform's equivalent) exists.
  - **On Vercel**: the package installs fine, but nothing in the compiled output statically references it — `pdfjs-dist` is in `serverExternalPackages`, so file tracing has no import chain to follow — and the function ships without the native binary. Distinctive log line: `Failed to load external module pdfjs-dist-<hash>/legacy/build/pdf.mjs: ReferenceError: DOMMatrix is not defined`. Fixed by three things that must stay together in `package.json`/`next.config.ts`: `@napi-rs/canvas` declared as a **direct** dependency (not just pdfjs's optional transitive one), listed in `serverExternalPackages` (prebuilt `.node` binaries can't be bundled), and force-traced into every function via `outputFileTracingIncludes`. Removing any one of them breaks production only — it will still work in dev.
- **pdfjs-dist server-side "fake worker" crash (Vercel only)**: distinct from the `DOMMatrix` bullet above, though it has the same root cause — a `serverExternalPackages` entry whose runtime-resolved file never gets traced. Symptom is a 500 on statement import with `Error: Setting up fake worker failed: "Cannot find module '/var/task/node_modules/pdfjs-dist/legacy/build/pdf.worker.mjs' imported from .../pdf.mjs"`. On Node, pdf.js always runs the worker on the main thread and reaches it via the *relative* string `"./pdf.worker.mjs"`, computed at runtime — invisible to file tracing, so the 2.3MB worker never ships. Never reproduces in dev. Fixed by two things that must stay together: `src/lib/importers/pdf-text.ts`'s `ensureWorker()` imports `pdfjs-dist/legacy/build/pdf.worker.mjs` by literal specifier and assigns it to `globalThis.pdfjsWorker` (pdf.js checks that *before* the dynamic import, and the literal is something the tracer can follow), plus a matching `outputFileTracingIncludes` entry as a safety net. `ensureWorker()` must run before the first `getDocument` — pdf.js memoizes the worker lookup via `shadow()`, so setting the global afterwards is too late. The worker build ships no types; `pdfjs-worker.d.ts` at the repo root declares it. To confirm a build actually carries the worker without deploying: `grep -o 'pdfjs-dist/legacy/build/[a-z.]*' '.next/server/app/(dashboard)/page.js.nft.json' | sort -u` must list `pdf.worker.mjs`, not just `pdf.mjs`.
...
- `src/lib/format-utils.ts` — the single source of truth for money formatting, in **both** directions, and both halves must stay in sync. `formatCurrency` renders pt-BR (`1234.5 → "1.234,50"`) via one shared `Intl.NumberFormat`; `parseAmountInput` is its inverse and the **only** place a typed amount becomes a number — it is wired into `amountSchema` (`src/lib/schemas.ts`) via `z.preprocess`, so every amount action gets it for free. Why it exists: amount fields are free text with `inputMode="decimal"`, a pt-BR keyboard produces `"685,91"`, and bare `z.coerce.number()` turns that into `NaN` → the Server Action throws → the form just sits there. Never normalize an amount ad hoc at a call site; `amountSearchTexts` exists so list search boxes match either separator without one. A comma is what disambiguates the separators, which leaves bare `"1.234"` reading as 1.234 — acceptable only because everything the app displays or prefills carries the comma.
...
- `src/lib/balance.ts` — `balanceTotals`, the one definition of "devido"/"pago" (optionally scoped to a month); see the `paid` note under "Prisma models".
...
- Unit tests in `src/lib/__tests__/`, mirroring `src/lib/` structure: `actions/` (one file per action module — `person`, `debt`, `payment`, `credit-card`, `statement`), `importers/`, `LLM-extract/`, `pdf/`, plus `balance.test.ts`, `date-utils.test.ts`, `installments.test.ts`, and `pdf-highlight.test.ts`.
...
- **Month carousel**: `month-carousel.tsx` is a controlled row of month chips (`months: string[]` of `"YYYY-MM"` keys from `date-utils.ts`'s `getAvailableMonths`, `selected`, `onSelect`), reused in two places. In `public-view.tsx`, one carousel sits above both the debts and payments lists and drives both via a single `selectedMonth` — it fully replaced the old date-range filter there. In the dashboard's `/person/[code]`, `person-detail-view.tsx` wraps `debts-section.tsx`/`payments-section.tsx` with the same carousel, passed down as an additional (not exclusive) filter — the existing `dateFrom`/`dateTo` inputs still work alongside it. **Both carousels also drive a `BalanceSummary` rendered directly below them**, via `balanceTotals(debts, payments, selectedMonth)` — an all-time total never matched the lists it sat above. It's positioned under the carousel rather than up beside the person's name on purpose: these are one month's totals, so they read as belonging to the month picker, not to the person. That's also why `person-detail-view.tsx` owns the person header (`EditablePersonHeader` + the Share/visibility/delete row) instead of `person/[code]/page.tsx` — the month state can't live in a Server Component. The one thing deliberately left on the all-time total is `public-view.tsx`'s `InstallmentCalculator`, which simulates paying off the whole balance. A debt belonging to an installment group shows a small "Parcela i/N" badge next to its title in every list/modal (`editable-debt.tsx`, `debt-detail-modal.tsx`, `public-view.tsx`).
...
- **Date entry is the one non-Radix primitive.** Every date field in the app is `src/components/date-field.tsx` (`react-aria-components`' `DateField`), never `<input type="date">`. Chrome renders a native date input in the **browser's** UI language and ignores the page's `lang="pt-BR"`, so a browser set to English showed `mm/dd/yyyy` with no HTML or CSS override available. `src/components/locale-provider.tsx` wraps the whole app in `<I18nProvider locale="pt-BR">` in `layout.tsx`, which is what makes the app — not the browser — decide the format. Radix has no date primitive; this is the justified exception to the Radix rule below.
  - It is deliberately a plain segmented field with **no calendar popover**: a floating layer here would have to coexist with `useDismiss` and `useInlineEditGuard` in the forms and dialogs these fields live in. Losing the native picker (including on mobile) was the accepted tradeoff. Don't "improve" it into a popover without re-reading the dismiss rules below.
  - It submits `YYYY-MM-DD`, so server actions and `dateSchema` are unchanged. With `required`, react-aria renders **two** inputs sharing the `name` — a `hidden required` validation sentinel plus a visually-hidden real one carrying `form=""` so it stays out of the FormData. `formData.get("date")` still returns one ISO string; don't "fix" the duplicate.
  - `src/components/date-field.test.tsx` locks the pt-BR segment order and the ISO submit value.
...
- **Money in, money out goes through `src/lib/format-utils.ts`** — `parseAmountInput` to read a typed amount, `formatCurrency` to display one. Never `Number(x)`, `parseFloat(x)` or an ad hoc `.replace(",", ".")` on an amount.
