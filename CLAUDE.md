...
- **Bradesco LLM hallucination guard**: Bradesco's LLM call is a strict pass-through JSON conversion of already-unambiguous `YYYY-MM-DD DESCRIPTION AMOUNT` lines (no interpretation needed) — but small models (e.g. `qwen2.5:3b`) can still fabricate an extra entry despite the "do NOT skip/filter/dedupe" system prompt, observed in practice as a duplicated real amount stamped with today's date instead of a real statement date. `bradesco.ts:extract()` whitelists the LLM's response against the actual pre-processed lines (matching by `date|amount`) and drops anything that doesn't correspond to a real line. This means Bradesco's LLM-corrections few-shot mechanism can't "recover" a transaction the pre-processing failed to produce as a clean line (any such recovery would also get filtered out) — an accepted trade-off since Bradesco's pre-processing is deterministic and already captures every real line (unlike the other banks, which rely on the LLM's own judgment over raw messy text and don't have this filter).
