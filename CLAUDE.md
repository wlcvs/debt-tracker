...
A parceled debt is just N `Debt` rows sharing one `installmentGroupId`, each with its own `installmentIndex`/`installmentTotal`, `paid`, and `date` — there's no separate "installment plan" model. `src/lib/installments.ts` holds the pure functions every server action and form preview calls, so they never drift:
...
- `buildInstallments(total, count, baseDate, direction?)` — the two above zipped into the `{ index, amount, date }[]` a group should consist of. Call this rather than pairing them by hand; `createDebt`, `updateDebtInstallmentGroup` and both forms' previews all go through it.
- `clampInstallments(raw)` + `MIN_INSTALLMENTS`/`MAX_INSTALLMENTS` — the count field's on-blur normalization, shared by `debt-form.tsx` and `installment-group-form.tsx` (see the `type="number"` note under "UI patterns" for why it must never run on `change`).
...
Once a debt belongs to a group, the UI treats it as a unit: in `debt-detail-modal.tsx` the edit button reads "Editar compra" and opens the group form instead of the per-debt one (there is no per-installment editing — editing one row in isolation is what would leave the group inconsistent), and the delete button calls `deleteDebtInstallmentGroup` instead of `deleteDebt`, removing the whole group. `installment-group-panel.tsx` (opened via "Ver parcelas" in the modal) lists every installment in a group and can bulk-mark a selection as paid (`toggleDebtsPaidBulk`), optionally also creating real `Payment` record(s) for the selected installments via `createPayment` — either one lump-sum payment or one per installment.

**Editing happens at the group's level, never one installment's.** `installment-group-form.tsx` (title, total, count, first date, description, method) calls `updateDebtInstallmentGroup`, and the panel shows it in place of the installment list. There is exactly one way in: "Editar compra" on the debt modal, which opens the panel with `startInEdit` so it lands straight on the form. "Ver parcelas" opens the list, which is only for ticking installments off — it deliberately carries no edit button of its own. Cancelling the form falls back to the list. That action **updates the surviving rows in place** rather than deleting and recreating the group, which is the only reason each installment's own `paid` flag survives an edit: shrinking the count deletes just the extra rows, growing it appends unpaid ones. Two consequences worth knowing:
- A `Payment` recorded for an installment that the edit removes stays behind — `Payment` has no FK to `Debt`, same as after `deleteDebtInstallmentGroup`. The form warns about this before saving.
- Editing invalidates the single installment the parent `DebtDetailModal` is still displaying, so the panel's `onSaved` closes that modal too.

There's no direction toggle on the edit form (unlike `createDebt`'s `installmentDirection`): `"backward"` exists to log an already-settled purchase from its last installment, and once the group exists, setting the first date does the same job with one control instead of two.
