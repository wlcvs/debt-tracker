...
**Editing happens at the group's level, never one installment's.** `installment-group-form.tsx` (title, total, count, first date, description, method) calls `updateDebtInstallmentGroup`, and the panel shows it in place of the installment list. Two ways in, one form: "Editar compra" on the debt modal opens the panel with `startInEdit`, landing straight on it; "Ver parcelas" opens the list, which has its own "Editar compra" button. Cancelling always falls back to the list. That action **updates the surviving rows in place** rather than deleting and recreating the group, which is the only reason each installment's own `paid` flag survives an edit: shrinking the count deletes just the extra rows, growing it appends unpaid ones. Two consequences worth knowing:
