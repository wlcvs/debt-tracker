...
- **Dismiss on outside-click / Escape**: use `src/lib/hooks/use-dismiss.ts`'s `useDismiss(ref, onDismiss, options?)` — never hand-roll a `document`/`window` listener effect for this, every component that needed "close on outside click" used to reimplement it slightly differently (some had Escape handling, some didn't). It attaches an outside-click listener (via `ref`) and/or an Escape-key listener (`window`) and calls `onDismiss`; either listener can be turned off individually (`outsideClick: false`, `escape: false`). It keeps `onDismiss` in a ref updated every render rather than as an effect dependency, so listeners are only (re)attached when `ref`/`enabled`/`outsideClick`/`escape` actually change, not on every state change the caller's `onDismiss` closes over. Used for simple dropdowns/panels (`method-select.tsx`, filter panels in `debts-section.tsx`/`payments-section.tsx`/`public-view.tsx`/`import-modal.tsx`'s `FilterToolbar`/`statements-modal.tsx`, `create-debt-form.tsx`) and for full-screen modals (`statements-modal.tsx`, `import-modal.tsx`), and for cancel-on-dismiss inline edits (`editable-person-header.tsx`).
  - **Nesting one dismissable inside another** (an inline edit inside a modal, e.g. renaming a statement inside `StatementsModal`, or editing a transaction's description inside `ImportModal`): the *outer* dismissable must check the *inner* one's state and cancel the inner edit instead of firing its own close, or a click/Escape meant only to cancel the inline edit will also close the whole modal. Don't pass `enabled: false` to the outer hook for this — its backdrop `onClick` and Escape listener still need to run, just with different logic depending on inner state. Instead, make the outer dismiss callback itself branch: `if (innerEditActive) cancelInnerEdit(); else outerClose();` (see `StatementsModal`'s `dismissModal`/`ImportModal`'s equivalent for the pattern). This bug class is easy to reintroduce when adding a new inline edit inside an existing modal — check for it.
