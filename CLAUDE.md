...
**`publicVisible` on Person** (default `true`, so existing shared links never break on migrate) gates `/public/[code]` entirely, at the data layer: `getDebtorViewById`'s `where` clause includes `publicVisible: true`, so a hidden person's row comes back `null` from the query itself and the route's existing `if (!debtor) notFound()` handles it with zero extra branching — the page and `PublicView` never need to know the flag exists. Toggled via `togglePersonPublicVisibility` (mirrors `toggleDebtPaid`'s find-then-flip shape), wired to a button in `person-visibility-toggle.tsx` on the dashboard's person page.
...
- **Dismiss on outside-click / Escape**: use `src/lib/hooks/use-dismiss.ts`'s `useDismiss(ref, onDismiss, options?)` — never hand-roll a `document`/`window` listener effect for this, every component that needed "close on outside click" used to reimplement it slightly differently (some had Escape handling, some didn't). It attaches an outside-click listener (via `ref`) and/or an Escape-key listener (`window`) and calls `onDismiss`; either listener can be turned off individually (`outsideClick: false`, `escape: false`). It keeps `onDismiss` in a ref updated every render rather than as an effect dependency, so listeners are only (re)attached when `ref`/`enabled`/`outsideClick`/`escape` actually change, not on every state change the caller's `onDismiss` closes over. Used for simple dropdowns/panels (`method-select.tsx`, filter panels in `debts-section.tsx`/`payments-section.tsx`/`public-view.tsx`/`import-modal.tsx`'s `FilterToolbar`/`statements-modal.tsx`, `create-debt-form.tsx`) and for full-screen modals (`statements-modal.tsx`, `import-modal.tsx`), and for cancel-on-dismiss inline edits (`editable-person-header.tsx`).
  - **Nesting one dismissable inside another** (an inline edit inside a modal, e.g. renaming a statement inside `StatementsModal`, or editing a transaction's description inside `ImportModal`): checking the inner edit's state (`if (innerEditActive) cancelInnerEdit(); else outerClose();`) from the outer dismiss callback is **not sufficient by itself** — if the inner edit commits/cancels via the input's `onBlur`, that state is *already reset* by the time the outer handler runs. Browser event order for a click that moves focus is `mousedown → blur → mouseup → click`: blur (and the `onBlur` commit it triggers) always fires *before* the click event reaches the outer element, so the outer handler's state check sees "no active edit" and closes anyway even though the click only meant to end the inline edit. Fix with `useDismissGuard()` (also in `use-dismiss.ts`): call its `suppressNext()` synchronously inside the inner edit's `onBlur` handler **and** its `onKeyDown`'s Escape branch (both — see the third failure mode below for why Escape needs this too), and wrap the outer dismiss callback's close logic in its `guard(() => ...)` — this swallows exactly the one outer-dismiss attempt that was actually the same gesture as the inner edit's own cancel (see `StatementsModal`'s `commitEdit`/`dismissModal` or `ImportModal`'s `commitEditingDesc`/`dismissModal` for the pattern). This bug class is easy to reintroduce when adding a new inline edit inside an existing modal — check for it, and don't assume a plain state check is enough. A modal's backdrop `onClick` must call the guarded dismiss callback, never the raw close function directly — wiring the backdrop straight to `onClose`/`handleClose` silently bypasses the guard entirely (found and fixed in `import-modal.tsx` while writing the Playwright suite in "Testing conventions" above — its backdrop was calling `handleClose` directly instead of `dismissModal`).
  - **A second, distinct nested-dismissable failure mode** (also found via the Playwright suite): `useDismiss`'s outside-click check must compare against `event.composedPath()`, not `event.target`. If a dismissable is nested *inside* another element that also has `useDismiss(ref, ..., { outsideClick: true })` (e.g. `MethodSelect`'s dropdown inside `CreateDebtForm`'s outer wrapper), and selecting an option closes the inner dismissable — removing its DOM node as a side effect of the very click that selected it — then by the time the outer `document`-level click listener runs, `event.target` is a *detached* node (React's own synthetic event handling, rooted higher in the tree, already committed the removal during the same bubble phase). `ref.current.contains(event.target)` then reads as `false` for a click that was actually inside `ref`, incorrectly firing the outer dismiss. `event.composedPath()` is captured at dispatch time and is immune to later DOM mutation, so it stays correct. This is unrelated to the blur-ordering bug above (no `blur`/`onBlur` involved at all) — it's specifically about *any* click handler that unmounts DOM inside a `useDismiss`-wrapped ancestor.
  - **A third nested-dismissable failure mode, specific to Escape** (found via manual QA during the `import-modal.tsx` breakup — CLAUDE.md previously claimed "Escape doesn't need the guard," which is wrong and has been corrected here): pressing Escape while editing calls the inline edit's own `onKeyDown` Escape branch directly (e.g. `setEditingDescIndex(null)`) — no blur involved at all. But React flushes that state update and re-renders **synchronously** for this discrete keydown event, including rebuilding the outer modal's dismiss callback, *before* the same native keydown event finishes bubbling to the `window`-level Escape listener `useDismiss` attaches. So the outer handler's rebuilt closure already sees the inline edit as inactive and wrongly closes the whole modal — same wrong outcome as the blur-ordering bug, via a third, independent mechanism (a synchronous-render race, not blur or DOM removal). Every inline edit's Escape branch needs its own `suppressNext()` call, exactly like its `onBlur` — omitting it (as every inline edit in this codebase originally did) reproduces this bug, and it predates any specific refactor: it was already on `main` before the `import-modal.tsx` breakup, just never caught because the Playwright suite only tested outside-click, not Escape, for these flows. `dismiss-behaviors.spec.ts` now covers both triggers for both modals.
